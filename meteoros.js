...
function Meteoro(posicao, r){
  if (posicao) {//se foi passado uma posição como parâmetro a posição passada é copida
      this.posicao = posicao.copy();
    } else {
      this.posicao = createVector(random(width), random(height));//se não uma posição aleatoria é gerada
    }
    if (r) {//se foi passado um raio como parâmetro o raio passado é dividido por 2
      this.r = r * 0.5;
    } else {
  ...
  this.offset = [];                                      //vetor que amazenará numeros aleatorios para ligar os pontos e dar forma ao contorno do meteoro
